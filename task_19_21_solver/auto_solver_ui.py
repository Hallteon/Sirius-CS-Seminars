...
class MoveDialog(QDialog):
    ...
    def on_move_type_changed(self, move_type):
        self.value_input.setVisible(move_type in ["Сложить", "Вычесть", "Умножить", "Разделить"])

    def validate_and_accept(self):
        move_type = self.move_type.currentText()

        if move_type == "Сложить":
            self.move_obj = AddMove(self.value_input.value())
        elif move_type == "Вычесть":
            self.move_obj = SubtractMove(self.value_input.value())
        elif move_type == "Умножить":
            self.move_obj = MultiplyMove(self.value_input.value())
        elif move_type == "Разделить":
            self.move_obj = DivideMove(self.value_input.value())
...
class MoveListWidget(QGroupBox):
    ...
    def init_ui(self):
        ...
        remove_btn = QPushButton("Удалить выбраный ход")
...
class ResultsWidget(QGroupBox):
    ...
    def update_results(self, results, full_analysis=None):
        text = f"<h3>Результаты игры</h3>"
        text += f"<b>Задание 19 (мин L1):</b> {results['19']}<br/>"
        text += f"<b>Задние 20 (первымя 2 W2):</b> {results['20']}<br/>"
...
class GameAnalyzerGUI(QMainWindow):
    ...
    def analyze_game(self):
        try:
            ...
            if not moves:
                QMessageBox.warning(self, "Error", "Пожалуйста, добавьте хоть одно действие (ну пожалуйста)")
                return

            print(terminal_condition, moves, self.settings_widget.s_min.value(),
                  self.settings_widget.s_max.value(), self._get_monotonic())
            ...
            game = Game(
                ...
                monotonic=self._get_monotonic(),
            )

            analyzer = Analyzer(game)
            results = analyzer.solve_19_20_21()
            full_analysis = analyzer.classify_up_to_k2()

            self.results_widget.update_results(results, full_analysis)

        except Exception as e:
            QMessageBox.critical(self, "AnalysisError", f"Ошибка анализа игры: {str(e)}")
